...
symbols <- c("EWC", "NUSI", "TCHP", "UBER", "VNM", "WEAT")

# Load data from 2020 to today
...
# plot the prices (close)
plot.xts(prices, legend.loc = 'topright')
...
head(x = round(asset_returns_xts, 4),  5)
...
# Add the box constraint that ensure the weights are between 0.05 and 0.6
min_var_portfolio <- PortfolioAnalytics::add.constraint(
  portfolio = min_var_portfolio,
...
# Examine
plot.xts(portfolio_returns_xts_rebalanced_monthly)
