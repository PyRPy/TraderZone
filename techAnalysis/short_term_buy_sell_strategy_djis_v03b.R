...
companylist <- read_csv("djis202408.csv") # prepared in excel
head(companylist)
...
stocks_high_switches <- subset(strategy_table,
                               cum_return > 0.0 & rate_of_switches > 0.75)

stocks_high_switches

stocks_potentials_sma_switch = subset(strategy_table,
                               cum_return > 0.0 & prob_drop_up >= 0.75 & price_close > SMA20 & rate_of_switches > 0.7)

stocks_potentials_sma_switch
