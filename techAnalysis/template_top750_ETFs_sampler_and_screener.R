...
library(dplyr)
companylist <- read_csv("ETFDB/etfdb_full_list.csv") # prepared in excel
colnames(companylist) <- c("Symbol", "ETF_name", "Asset_class", 
                           "Total_assets", "YTD_Price_Change", "Daily_Volume", 
                           "Close_price", "Pro")
head(companylist)
# clean up
companylist["Pro"] <- NULL
companylist$Total_assets <- as.numeric(gsub('[$,]', '', companylist$Total_assets))
companylist$Asset_class <- as.factor(companylist$Asset_class)

table(companylist$Asset_class)
companylist <- companylist %>% 
  filter(Total_assets >= 1000 & Asset_class == "Equity")
...
idx_gold <- grepl('Gold', companylist$ETF_name) & !grepl('Goldman', companylist$ETF_name)
idx_clean <- grepl('Clean', companylist$ETF_name) | grepl('Solar', companylist$ETF_name) | grepl('Battery', companylist$ETF_name)
...
sampledStocks <- stockSymbols[idx, "Symbol"] # add idx with sampling
sampledStocks
